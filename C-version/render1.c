...
static const attractor attractors[3] = {
    {0.31 * WIDTH, 0.36 * HEIGHT, 1, {255, 164, 0}},
    {0.81 * WIDTH, 0.45 * HEIGHT, 1, {0, 159, 253}},
    {0.42 * WIDTH, 0.65 * HEIGHT, 1, {42, 42, 114}}};
...
lutData calculateLutData(point point) {
  ...
  for (unsigned int i = 0; i < sizeof(attractors) / sizeof(attractor); i++) {
    ...
    const float attractionStrength = attractor.strength / powf(distance, 1.8f);
...
color calculateColor(point point, lutData lut[]) {
  ...
  while (i < ITERATION) {
    ...
    return (color){128 + lutData.dx * 100, 128 + lutData.dy * 100,
                   50 + -lutData.dy * 50};
