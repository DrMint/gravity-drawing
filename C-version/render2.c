...
static const float SIMULATION_PRECISION_MULTIPLIER = 4.0f;
...
lutData calculateLutData(point point) {
  ...
  for (unsigned int i = 0; i < sizeof(attractors) / sizeof(attractor); i++) {
    ...
    if (distance < DISTANCE_TRESHOLD) {
      attractorColor = attractor.color;
    }
...
color calculateColor(point point, lutData lut[]) {
  ...
  while (i < ITERATION) {
    ...
    if (!isEmptyColor(lutData.color)) {
      return (color){lutData.color.r + 5 * dx - 65, lutData.color.g - 2 * dy,
                     lutData.color.b + dy};
    }
    dx += lutData.dx;
    dy += lutData.dy;
