...
static const float SIMULATION_PRECISION_MULTIPLIER = 11.0f;
...
static const unsigned int ITERATION = 180 * SIMULATION_PRECISION_MULTIPLIER;
...
static const attractor attractors[3] = {
    {0.31 * WIDTH, 0.36 * HEIGHT, 1, {210, 180, 59}},
    {0.81 * WIDTH, 0.45 * HEIGHT, 1, {239, 241, 237}},
    {0.42 * WIDTH, 0.65 * HEIGHT, 1, {118, 97, 150}}};
...
lutData calculateLutData(point point) {
  ...
  return (lutData){dx, dy, attractorColor};
}
...
color calculateColor(point point, lutData lut[]) {
  ...
  while (i < ITERATION) {
    ...
    if (lutData.colorIndex >= 0 && lutData.colorIndex < 3) {
      const float colorMultiplier = 7.0f;
      const unsigned int a = 255 - (powf(dx, 2.0f) * colorMultiplier +
                                    powf(dy, 2.0f) * colorMultiplier * 0.56f);
      const unsigned int b = i * 255 / ITERATION;
      if (lutData.colorIndex == 0) {
        return (color){0, a, b};
      }
      if (lutData.colorIndex == 1) {
        return (color){a, b, 0};
      }
      if (lutData.colorIndex == 2) {
        return (color){b, 0, a};
      }
    }
